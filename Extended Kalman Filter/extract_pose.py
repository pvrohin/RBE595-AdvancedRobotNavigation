...
def estimate_pose(data, tag_coordinates):
    ...
    data['p1'] = data['p1'].T.reshape(-1, 2)

    data['p2'] = np.array(data['p2'])

    data['p2'] = data['p2'].T.reshape(-1, 2)

    data['p3'] = np.array(data['p3'])

    data['p3'] = data['p3'].T.reshape(-1, 2)

    data['p4'] = np.array(data['p4'])

    data['p4'] = data['p4'].T.reshape(-1, 2)
    ...
    #tvec = -rot_matrix.T @ tvec + tvec_imu_camera
    tvec = tvec + tvec_imu_camera

    print(tvec.shape)
    
    #return tvec_imu_camera.flatten(), np.array([roll, pitch, yaw])
    return tvec.flatten(), np.array([roll, pitch, yaw])
...
def world_corners():
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            corner1 = (x, y)
            corner2 = (x + tag_size, y)
            corner3 = (x + tag_size, y + tag_size)
            corner4 = (x, y + tag_size)
